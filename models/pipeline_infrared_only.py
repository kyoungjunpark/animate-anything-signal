...
class MaskStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        infrared_images = self.image_processor.preprocess(infrared_images, height=height, width=width)
        infrared_images = self._encode_vae_image(infrared_images, device, num_videos_per_prompt, False) * self.vae_scale_factor
        infrared_images = infrared_images.to(dtype)
        infrared_images = rearrange(infrared_images, '(b f) c h w-> b f c h w', b=batch_size).to(dtype)

        # infrared_images = infrared_images.unsqueeze(0).to(device)

        infrared_embeddings = video_encoder(infrared_images) / self.vae_scale_factor
        infrared_hidden_embeddings = video_encoder_hidden(infrared_images)  / self.vae_scale_factor

        infrared_embeddings = infrared_embeddings.repeat(1, num_frames, 1, 1, 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([final_input, latent_model_input, condition_latent], dim=2).to(dtype)
