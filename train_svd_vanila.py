...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    #  config = ProjectConfiguration(project_dir=output_dir, logging_dir=output_dir + "/logs")
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("svd_vanila with random data & random start")
...
def eval(pipeline, vae_processor, validation_data, out_file, index, forward_t=25, preview=True):
    ...
    for image in validation_data.prompt_image:
...
def main_eval(
    ...
    output_dir = "output/svd_vanila_random"
    iters = 5
