...
class cdfvd(object):
    ...
    def load_videos(self, video_info: str, video_data: None, resolution: int = 256, sequence_length: int = 16, sample_every_n_frames: int = 1,
        ...
        `c` loads videos from a folder containing video files.
        ...
        if data_type=='video_torch':
            video_loader = [{'video':  rearrange(video_data[i:i+batch_size], 'b t c h w -> b c t h w')} for i in range(0, video_data.size(0), batch_size)]
        elif video_info.endswith('.pt'):
            video_loader = [{'video':  rearrange(video_data[i:i+batch_size], 'b t c h w -> b c t h w')} for i in range(0, video_data.size(0), batch_size)]
