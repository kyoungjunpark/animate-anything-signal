...
from utils.pips.utils import basic, samp, misc, improc
...
def balanced_ce_loss(pred, gt, valid=None):
    ...
    pos_loss = basic.reduce_masked_mean(loss, pos*valid)
    neg_loss = basic.reduce_masked_mean(loss, neg*valid)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fhid, fcorr, flow):
        B, S, D = flow.shape
        assert(D==3)
        flow_sincos = misc.get_3d_embedding(flow, 64, cat_coords=True)
...
class Pips(nn.Module):
    ...
    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        ...
        if feat_init is None:
            ...
            ffeat = samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B, N, C
        else:
            ffeat = feat_init
        ...
        if sw is not None and sw.save_this:
            ...
            for s in range(S):
                if trajs_g is not None:
                    ...
                    kp = improc.draw_circles_at_xy(torch.cat([e_, g_], dim=1), H8, W8, sigma=1).squeeze(2)
                    kp = improc.seq2color(kp, colormap='onediff')
                else:
                    kp = improc.draw_circles_at_xy(coords[0:1,s,0:1], H8, W8, sigma=1).squeeze(2)
                    kp = improc.seq2color(kp, colormap='spring')
                kp = improc.back2color(kp)
                kp_vis.append(kp)
