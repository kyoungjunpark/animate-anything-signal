import torch
import numpy as np
from utils.pips.utils import basic
...
def reduce_emb(emb, valid=None, inbound=None, together=False):
    ...
    reduced_emb = basic.normalize(reduced_emb) - 0.5
...
def xy2heatmaps(xy, Y, X, sigma=30.0):
    ...
    grid_y, grid_x = basic.meshgrid2d(B, Y, X, device=device)
...
def draw_frame_id_on_vis(vis, frame_id, scale=0.5, left=5, top=20):
    ...
    frame_str = basic.strnum(frame_id)
...
COLORMAP_FILE = "./utils/pips/utils/bremm.png"
class ColorMap2d:
        ...
        if self.save_this:
            ...
            if feats.ndim==6:
                ...
                if valids is None:
                    ...
                    feats = basic.reduce_masked_mean(feats, valids, dim=reduce_dim)
        ...
        if self.save_this:
            if feat.ndim==5: # B x C x D x H x W
                ...
                if valid is None:
                    ...
                    feat = basic.reduce_masked_mean(feat, valid, dim=reduce_axis)
